...
BUILDIN_FUNC(delequip)
{
	...
	st->state = END;
	clif_scriptclose(sd,st->oid);
	return 1lif_emotion(&sd->hd->bl,E_SWT);
	}
	return 0;
}
 ...
 *--------------------------------------- = NULL;
	...
	if( vclass == JOB_WEDDING r t_getnum(st,2);
	if(vclass == JOB_WEDDING)
	{
		if (!battle_config.wedding_modifydisplay || //Do not show) //Baby classes screw up when showing wedding sprites [Skotlex] They don't seem to anymore
		return 0;
	}
...
BUILDINoom)
{
	st *title = script_getstr(st, 2);
	int limit = script_getnum(st, 3);
	const char * limit = script_getnum(st, 3);
...
/// kickwaitingroomall;
BUILDIN_FUNC(waitingroomkickal *cd;
	...
	if( struct npc_data *)map_id2bl(st->oid);
...
BUILDIN_FUNC(enablewaitingroomeven *
{
	struct npc_data* nd;
	struct chat_data* cd;
...
BUILDIN_FUNC(addrid)
{
	...
	iter = mapit_getallusers();
	switch(script_g *)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC irst(iter); mapit_exists(iter); sd = (TBL_PC*)mapit_next(iter)) {
				if(!script_getnum(st,3) || !sd->st)
					if(sd->status.account_id != st->rid) //Attached player already runs.
						run_script(st->script,st->pos,sd->status.account_id,st->oid);
 *)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC irst(iter); mapit_exists(iter); sd = (TBL_PC*)mapit_next(iter)) {
				if(!script_getnum(st,3) || !sd->st)
		...
		case 2:
			if(script_getnum(st,4) == 0) {
				script_pushint(s *)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC irst(iter); mapit_exists(iter); sd = (TBL_PC*)mapit_next(iter)) {
				if(!script_getnum(st,3) || !sd->st)
		...
		case 3:
			if(script_getnum(st,4) == 0) {
				...
				if(!script_getnum(st,3) || !sd->st)
					//Attached player already runs.
					if((sd->sguild.account_id != st->rid) && (sd->status.party_id == script_getnum(st,4)))
						run_script(st->script,st->pos,sd->status.account_id,st->oid);
			}
			break;
		case 4:
...
/* PVP timer handling */
static int script_mapflag_pvp_sub(struct block_ list *bl,va_list ap) {
	TBL_PC* sd = (TBL_PC*)bl;
...
BUILDIN_FUNC(setmapflag)
{
	...
	if(m >= 0) {
		switch(i) {
			...
			case MF_NOBASEEXP:		map[OBEXP:		map[m].flag.nojobexp = 0; break;
...
BUILDIN_FUNC(guardianinfo)
{
	...
	struct guild_castle* gc = *uild_mapname2gc(mapname);
	struct mob_data* gd;
 ...
 = script_rid2sd(st);
	...
	for (i = 0; i < MAX_CART; i++) {
		if (sd->status.cart[i].nameid > 0 && sd->status.cart[i].amount > 0) {
			pc_setreg(sd,reference_uid(add_str("@cartinventorylist_id"),j),sd->status.cart[i].nameid);
			pc_setreg(sd,reference_uid(add_str("@cartinventorylist_amount"),j),sd->status.cart[i].amount);
			pc_setreg(sd,reference_uid(add_str("@cartinventorylist_equip"),j),sd->status.cart[i].equip);
			pc_setreg(sd,reference_uid(add_str("@cartinventorylist_refine"),j),sd->status.cart[i].refine);
			pc_setreg(sd,reference_uid(add_str("@cartinventorylist_identify"),j),sd->status.cart[i].identify);
			pc_setreg(sd,reference_uid(add_str("@cartinventorylist_attribute"),j),sd->status.cartorylist_attribute"), j),sd->status.inventory[i].attribute);
			for (k = 0;"@cartinventorylist_card%d",k + 1);
				pc_setreg(sd,reference_uid(add_str(card_var),j),sd->status.cart[i].card[k]);
			}
			pc_setreg(sd,reference_uid(add_str("@cartinventorylist_expire"),j),sd->status.cart[i].expire_time);
			pc_setreg(sd,reference_uid(add_str("@cartinventorylist_bound"),j),sd->status.cart[i].bound);
			j++;
		}
	}
	pc_setreg(sd,add_str("@cartnd);
			j++;
		}
	}
...
BUILDIN_FUNC(npcwalkto)
{
	...
	if(nd) {
		if(!nd->status.hp)
			...
			status_calc_npc(nd,SC	return SCRIPT_CMD_SUCCESS	uni
//Slktoxy(&nd->bl,x,y,0);
	}
...
// stop an npc's movement [Valaris]
 = (struct npc_data *)map_id2bl(st->oid);
	...
	if( nd truct npc_data *)map_id2bl(st->oid);	return SCRIPT_CMD_SUCCESS		uni//Set click npc distance [4144]
...
BUILDIN_FUNC(getnpcclass)
{
	...
	script_pushint(st,(int)nd->class_ay*	return SCRIPT_CMD_SUCCESSskill_bonus_timer, sd->bl.id, 0);

	return 0;
}

/*==getlook char info. getlook(arg)ave point. argument: 0- map name, 1- x, 2- y
 *------------------------------------------*/
BUILDIN_
	sd = script_rid2sd(st);
	if( sd == NULL )
...
/*==========================NPC/pet/hom/merc/elem=====
  ...
  *                                1 - NPC coord
  *                                2 - Pet coord
  *   see 'getunitdata'                 3 - Mob coord (not released)
  *                                4 - Homun coord
  *                                5 - Mercenary coord
  *                                6 - Elemental coord
...
Possible needly check function parameters on C_STR,C_INT,C_INT
	...
	//Return SSCRIPT_CMD_SUCCESSccess // Returns the map name of given map ID
///
/// mapid2name <map ID>;
BUILDIN_FUNC(mapid2name)
{
	...
	script_pushstrcopy(st,map_mapid2mapname(m));t	return SCRIPT_CMD_SUCCESSelse
			script_pushint(st,-1);
	}
...
BUILDIN_FUNC(autoequip)
{
	...
	if( !itemdb_isequip2(item_data)hu) {
		ShowError("buildin_utoequip: Item '%dSCRIPT_CMD_FAILURE;
	}

	item_data->flag.autoequip = (flag > 0 ? 1 : 0);
	return SCRIPT_CMD_SUCCESSdata-/*=======================================================
 * Equip2
 * equip2 <item id>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>;or an
 * item with inventory index specified
 *---------------------------equip2)
{
	int i;
...
BUILDIN_FUNC(substr)
{
	...
	if(start >= 0 && end < strlen(str) && start <= end) *)aMalloc(len + 1);
		memcpy(output, &str[start], len);
	} else
		output = (char  &str[start], len);
	} else
...
/// setnpcdisplay("<npc name>", <new class id>) -> <in *name;
	const char *cdisplay)
{
	const char* name;
	...
	switch( bl->type ) {
		...
		case BL_HOM:
			...
			switch( type ) {
				...
				case 21: hd->ud.immune_attack = (value > 0 ? true : false); break;
				default:
		...
		case BL_PET:
			...
			switch( type ) {
				...
				case 19: pd->ud.immune_attack = (value > 0 ? true : false); break;
				default:
		...
		case BL_MER:
			...
			switch( type ) {
				...
				case 18: mc->ud.immune_attack = (value > 0 ? true : false); break;
				default:
		...
		case BL_ELEM:
			...
			switch( type ) {
				...
				case 20: ed->ud.immune_attack = (value > 0 ? true : false); break;
				default:
...
BUILD *_FUNC(waitingroom2bg_single)
{
	const char* map_name;
	struct npc_data *nd;
	struct chat_data *cd;
...
BUILDIN_FUNC(bg_war *map_namesession_data *sd;
	int x, y, mapindex, bg_id;
 ...
 *---------------------------------------_party)
{
	...
	if( !(p = party_searc(party_id)) ) {
		script_pushint(st, ); // Returns false if party does not exist.
		ret {
		struct map_session_data *pl_sd = p->data[i].sd;
 ...
 * amount : Amount of needed Guild Mrameter]
 * amount : Amount of needed Partymembers for the Instance. [Optional Parameter]
 * min : Minimum Level needed to join the Instance. [Optional Parameter]
 * max : Maxium Level allowed to join the Instance. [Optguild (getcharid(2){,amount}{,min}{,max});
...
{
	unsigned int ty *nsigned int uses;
	struct map_session_data* sd;
...
BUILDIN_FUNC(checkbound) {
	...
	unsigned short nameid = script_getnum(st,2)UNC(countbound)
{
	int i, type, j = 0, k = 0;
	TBL_PC *sd;

	if( (sd =if( !(itemdb_exists(nameid)) ) {
