...
st,3);
	...
	if( tick != 0 ) {
		...
		if( battle_config.mon_trans_disable_in_gvg && map_flag_gvg2(sd->bl.m) ns_disable_in_gvg && map_flag_gvg2(sd->bl.m)) {
			c lif_displaymessage(sd->fd,msg_txt(1493)); //Transforming into monster is ot a( sd->disguise d Wars.
			return 0;
		}

		if (sd->disguise) {
			c lif_displaymessage(sd->fd,msg_txt(1491)); //Cannot transform into onsorm!!
...
BUILDIN_FUNC(countspiritball) {
	...
	return SCRIPT_CMD_SUCCESS,100,/** Merges separated stackable items because of guid
 * mergeitem {<item_id>};
 * mergeitem {"<item name>"};
